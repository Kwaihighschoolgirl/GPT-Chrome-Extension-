...
// content.js에서 오는 메시지 처리 (추가된 부분)
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'translate' || request.action === 'summarize') {
    handleFloatingButtonRequest(request, sender, sendResponse);
    return true; // 비동기 응답을 위해 true 반환
  }
});

// 플로팅 버튼 요청 처리 함수 (추가된 부분)
async function handleFloatingButtonRequest(request, sender, sendResponse) {
  const { apiKey } = await chrome.storage.sync.get("apiKey");
  ...
  try {
    const result = await callGPT(prompt, apiKey);
    await saveToHistory(taskType, request.text, result);
...
console.log("GPT Helper background script loaded.");
